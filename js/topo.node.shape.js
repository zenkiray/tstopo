      ...
      switch (shape) {
        ...
        case 'circle':
          shapeCreator = d3.create('svg:circle')
            .attr('r', (nodeData.getSize() || nodeData.getWidth() / 2) + offsetSize)
            .attr('cx', nodeData.getCx() || 0)
            .attr('cy', nodeData.getCy() || 0);
          break;
        default:
          shapeCreator = d3.create('svg:path')
            .attr('d', NodeShape.drawShapePath(nodeData, offsetSize))
      ...
      switch (nodeData.shape) {
        ...
        case 'ellipse':
          point1 = this.intersect[nodeData.shape]({
            x: nodeData.x,
            y: nodeData.y
          }, nodeData.width / 2, nodeData.height / 2, point);
          break;
        case 'polygon':
          point1 = this.intersect[nodeData.shape]({
            x: nodeData.x,
